...
function Track() {
  $.get({
    ...
    if (error && !data) {
      $.log(error);
      $.msg("Apple Order Track", "访问失败！", error)
    } else {
      if (response.statusCode != 200) {
        $.msg("Apple Order Track", "", "访问失败，请检查！")
      } else {
        ...
        if ($.getdata(order_time_key)) {
          if (order_time != $.getdata(order_time_key)){
            $.msg("Apple Order Track", "送达日期更新", order_time)
            $.setdata(order_time,order_time_key)            
          }
        }else{
          $.setdata(order_time,order_time_key)
          $.msg("Apple Order Track", "首次录入时间", order_time)
        }
        if ($.getdata(order_status_key)) {
          if (order_status != $.getdata(order_status_key)){
            $.msg("Apple Order Track", "订单状态更新", order_status)
            $.setdata(order_status,order_status_key)            
          }
        }else{
          $.setdata(order_status,order_status_key)
          $.msg("Apple Order Track", "首次录入订单状态", order_status)
        }        
      }
    }
    $.done();
